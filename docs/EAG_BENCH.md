...
### T9 (observability export)

OTel exporter shipped, scores **2/2**: governance events round-trip to OTel records, and the enforcement path is **statically proven never to import it** (so an exporter failure cannot affect a decision, and enforcement stays zero-egress).
